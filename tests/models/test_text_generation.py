...
@pytest.mark.skipif(
    sys.platform == "win32", reason="Integer division bug in HuggingFace on Windows"
)
def test_call_function_text_generation():
    ...
    text_generation_for_pretrained_model = shap.models.TextGeneration(
        model, tokenizer=tokenizer, device="cpu"
    )
    text_generation_for_model_agnostic_scenario = shap.models.TextGeneration(
        f, device="cpu"
    )
    ...
    target_sentence_for_pretrained_model = [
        ...
        for g in target_sentence_ids_for_pretrained_model
    ]
    target_sentence_for_model_agnostic_scenario = (
        text_generation_for_model_agnostic_scenario(s)
    )

    assert (
        target_sentence_for_pretrained_model[0]
        == target_sentence_for_model_agnostic_scenario[0]
    )
