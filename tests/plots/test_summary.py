...
def test_summary():
    ...
    fig = plt.figure()
    shap.summary_plot(rng.standard_normal(size=(20, 5)), show=False, rng=rng)
...
def test_summary_with_data():
    ...
    rng = np.random.default_rng(0)
    fig = plt.figure()
    shap.summary_plot(rng.standard_normal(size=(20, 5)), rng.standard_normal(size=(20, 5)), show=False, rng=rng)
    fig.set_layout_engine("tight")
    return fig
...
def test_summary_multi_class():
    ...
    fig = plt.figure()
    shap.summary_plot(
        [rng.standard_normal(size=(20, 5)) for i in range(3)], rng.standard_normal(size=(20, 5)), show=False, rng=rng
    )
    fig.set_layout_engine("tight")
    return fig
...
def test_summary_multi_class_legend_decimals():
    ...
    fig = plt.figure()
    shap.summary_plot(
        [rng.standard_normal(size=(20, 5)) for i in range(3)],
...
def test_summary_multi_class_legend():
    ...
    fig = plt.figure()
    shap.summary_plot(
        [(2 + rng.standard_normal(size=(20, 5))) for i in range(3)],
        2 + rng.standard_normal(size=(20, 5)),
        show=False,
        show_values_in_legend=True,
        rng=rng,
    )
...
def test_summary_bar_with_data():
    ...
    fig = plt.figure()
    shap.summary_plot(
        rng.standard_normal(size=(20, 5)), rng.standard_normal(size=(20, 5)), plot_type="bar", show=False, rng=rng
    )
    fig.set_layout_engine("tight")
    return fig
...
def test_summary_dot_with_data():
    ...
    fig = plt.figure()
    shap.summary_plot(
        rng.standard_normal(size=(20, 5)), rng.standard_normal(size=(20, 5)), plot_type="dot", show=False, rng=rng
    )
    fig.set_layout_engine("tight")
    return fig
...
def test_summary_compact_dot_with_data():
    ...
    shap_values = rng.standard_normal(size=(n_samples, n_features, n_features))
    fig = plt.figure()
...
def test_summary_with_log_scale():
    ...
    rng = np.random.default_rng(0)
    fig = plt.figure()
    shap.summary_plot(rng.standard_normal(size=(20, 5)), use_log_scale=True, show=False, rng=rng)
    fig.set_layout_engine("tight")
    return fig
...
def test_summary_plot_interaction():
    ...
    shap_values = rng.standard_normal(size=(n_samples, n_features, n_features))
    feature_names = [f"Feature {i + 1}" for i in range(n_features)]
    X = pd.DataFrame(rng.standard_normal(size=(n_samples, n_features)), columns=feature_names)
...
def test_summary_plot_twice():
    ...
    shap.summary_plot(shapValues, X, show=False, rng=np.random.default_rng(0))
    shap.summary_plot(shapValues, X, show=False, rng=np.random.default_rng(0))
    fig = plt.gcf()
    fig.set_layout_engine("tight")
    return fig
...
def test_summary_plot_wrong_features_shape():
    ...
    with pytest.raises(ValueError, match=emsg):
        shap.summary_plot(rng.standard_normal(size=(20, 5)), rng.standard_normal(size=(20, 4)), show=False, rng=rng)
    ...
    with pytest.raises(AssertionError, match=emsg):
        shap.summary_plot(rng.standard_normal(size=(20, 5)), rng.standard_normal(size=(20, 1)), show=False, rng=rng)
