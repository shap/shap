...
def test_force_array_higher_predictions_first():
    """Test that stacked force plots keep the higher prediction first (GH #4342)."""
    plot = shap.plots.force(0.0, np.array([[-2.0, -2.0], [2.0, 2.0]]))

    out_values = [exp["outValue"] for exp in plot.data["explanations"]]
    sim_indices = [exp["simIndex"] for exp in plot.data["explanations"]]

    assert out_values[sim_indices.index(1)] > out_values[sim_indices.index(2)]
