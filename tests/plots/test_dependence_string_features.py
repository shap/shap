...
def test_approximate_interactions():
    ...
    X_two_string_features = _create_sample_dataset(
        string_features={"Sex", "Blood group"}
    )

    shap_values = np.random.randn(*X_one_string_feature.values.shape)

    interactions_no_features = shap.approximate_interactions(
        0, shap_values, X_no_string_features
    )
    interactions_one_string_feature = shap.approximate_interactions(
        0, shap_values, X_one_string_feature
    )
    interactions_two_string_feature = shap.approximate_interactions(
        0, shap_values, X_two_string_features
    )
