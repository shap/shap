...
def test_multioutput_decision(values_features):
    ...
    shap.multioutput_decision_plot(base_values_list,
...
def test_multioutput_decision_raises(values_features):
    ...
    with pytest.raises(ValueError, match="The base_values and shap_values args expect lists."):
        shap.multioutput_decision_plot(adult_rfc_shap_values.base_values[0, :],
                                       adult_rfc_shap_values.values[:, :, :],
                                       row_index=0,
                                       features=X,
                                       )
    with pytest.raises(ValueError, match="The shap_values arg should be a list of two or three dimensional SHAP arrays."):
        adult_rfc_shap_values_list = [adult_rfc_shap_values.values[:, 0, i] for i in range(adult_rfc_shap_values.shape[2])]
        base_values_list = list(adult_rfc_shap_values.base_values[0, :])
        shap.multioutput_decision_plot(base_values_list,
                                       adult_rfc_shap_values_list,
                                       row_index=0,
