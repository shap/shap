...
def test_update():
    ...
    shap.benchmark.update(
        model, X, y, explainer, masker, sort_order, score_function, perturbation, scores
    )
...
def test_get_metrics():
    ...
    scores1 = {
        "name": "test1",
        "metrics": ["keep positive", "keep absolute"],
        "values": dict(),
    }
    scores2 = {
        "name": "test2",
        "metrics": ["keep positive", "keep negative"],
        "values": dict(),
    }
    ...
    assert (
        set(shap.benchmark.get_metrics(benchmarks, lambda x, y: x.intersection(y)))
        == expected_metrics1
    )
    assert (
        set(shap.benchmark.get_metrics(benchmarks, lambda x, y: x.union(y)))
        == expected_metrics2
    )
