...
def test_init():
    ...
    sequential_perturbation = benchmark.SequentialPerturbation(model, tabular_masker, sort_order, perturbation)
    ...
    sequential_perturbation = benchmark.SequentialPerturbation(model, text_masker, sort_order, perturbation)
    assert sequential_perturbation.data_type == "text" 

    image_masker = Image("inpaint_telea")
    sequential_perturbation = benchmark.SequentialPerturbation(model, image_masker, sort_order, perturbation)
    assert sequential_perturbation.data_type == "image" 

    fc_masker = FixedComposite(text_masker)
    sequential_perturbation = benchmark.SequentialPerturbation(model, fc_masker, sort_order, perturbation)
    assert sequential_perturbation.data_type == "text" 
