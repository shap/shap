...
def test_tied_pair():
    ...
    explainer = shap.LinearExplainer(
        (beta, 0), (mu, Sigma), feature_dependence="correlation"
    )
...
def test_tied_pair_independent():
    ...
    X = np.ones((1, 3))
    explainer = shap.LinearExplainer(
        (beta, 0), (mu, Sigma), feature_dependence="independent"
    )
...
def test_tied_pair_new():
    ...
    explainer = shap.explainers.Linear(
        (beta, 0), shap.maskers.Impute({"mean": mu, "cov": Sigma})
    )
    assert np.abs(explainer.shap_values(X) - np.array([0.5, 0.5, 0])).max() < 0.05
...
def test_sparse():
    ...
    make_multilabel_classification = pytest.importorskip(
        "sklearn.datasets"
...
def test_feature_perturbation_sets_correct_masker(feature_pertubation, masker):
    ...
    explainer = shap.explainers.Linear(
        model, X, feature_perturbation=feature_pertubation
    )
