...
def test_interactions():
    ...
    common.test_interactions_additivity(
...
def test_tabular_single_output_partition_masker():
    ...
    common.test_additivity(
        shap.explainers.Exact, model.predict, shap.maskers.Partition(data), data
    )
...
def test_tabular_multi_output_partition_masker():
    model, data = common.basic_xgboost_scenario(100)
    common.test_additivity(
        shap.explainers.Exact, model.predict_proba, shap.maskers.Partition(data), data
    )
...
def test_tabular_single_output_independent_masker():
    model, data = common.basic_xgboost_scenario(100)
    common.test_additivity(
        shap.explainers.Exact, model.predict, shap.maskers.Independent(data), data
    )
...
def test_tabular_multi_output_independent_masker():
    model, data = common.basic_xgboost_scenario(100)
    common.test_additivity(
        shap.explainers.Exact, model.predict_proba, shap.maskers.Independent(data), data
    )
