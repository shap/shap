...
def test_exact_second_order():
    ...
    def model(data):
        return (
            data[:, 0] * data[:, 2] + data[:, 1] + data[:, 2] + data[:, 2] * data[:, 3]
        )
...
def test_tabular_single_output_partition_masker():
    ...
    common.test_additivity(
        shap.explainers.Permutation, model.predict, shap.maskers.Partition(data), data
    )
...
def test_tabular_single_output_independent_masker():
    model, data = common.basic_xgboost_scenario(100)
    common.test_additivity(
        shap.explainers.Permutation, model.predict, shap.maskers.Independent(data), data
    )
