...
def test_pyspark_regression_decision_tree():
    ...
    for regressor in regressors:
        ...
        explainer = shap.TreeExplainer(model)
        X = pd.DataFrame(data=iris_sk.data, columns=iris_sk.feature_names).drop('sepal length (cm)', 1)[
...
def test_provided_background_tree_path_dependent():
    ...
    assert np.abs(
        explainer.expected_value - bst.predict(dtrain, output_margin=True).mean()) < 1e-6, "Bad expected_value!"
...
def test_provided_background_independent():
    ...
    assert np.max(np.abs(diffs)) < 1e-4, "SHAP values don't sum to model output!"
    assert np.abs(
        explainer.expected_value - bst.predict(dtrain, output_margin=True).mean()) < 1e-4, "Bad expected_value!"
