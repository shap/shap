...
def test_tree_ensemble_predict_log_loss_validates_labels():
    ...
    model = GradientBoostingRegressor(n_estimators=5, max_depth=2, random_state=0).fit(X, y)
    ...
    np.testing.assert_allclose(explainer.model.predict(X[:5], y=y[:5]), (model.predict(X[:5]) - y[:5]) ** 2)
