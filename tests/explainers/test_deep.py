...
def test_tf_eager():
    ...
    model.add(
        tf.keras.layers.Dense(y.shape[1], input_shape=(10,), activation="softmax")
    )
    model.summary()
    ...
    assert (
        np.abs(e.expected_value[0] + sv[0].sum(-1) - model(x.values)[:, 0]).max() < 1e-4
    )
...
def test_tf_keras_mnist_cnn():  # pylint: disable=too-many-locals
    ...
    model = Sequential()
    model.add(Conv2D(2, kernel_size=(3, 3), activation="relu", input_shape=input_shape))
    ...
    assert d / np.abs(diff).sum() < 0.001, (
        "Sum of SHAP values does not match difference! %f" % d
    )
...
def test_tf_keras_imdb_lstm():
    ...
    sums = np.array([shap_values[i].sum() for i in range(len(shap_values))])
    diff = sess.run(mod.layers[-1].output, feed_dict={mod.layers[0].input: testx})[
        0, :
    ] - sess.run(
...
def test_pytorch_custom_nested_models():
    ...
    data = TensorDataset(torch.tensor(X).float(), torch.tensor(y).float())
...
def test_pytorch_single_output():
    ...
    class Net(nn.Module):
        ...
        def forward(self, X):
            ...
            return self.maxpool2(self.linear(self.leaky_relu(x)).unsqueeze(1)).squeeze(
                1
            )
...
def test_pytorch_multiple_inputs():
    ...
    def _run_pytorch_multiple_inputs_test(disconnected):
        ...
        x2 = X[:, : num_features // 2]
        data = TensorDataset(
            torch.tensor(x1).float(), torch.tensor(x2).float(), torch.tensor(y).float()
        )
        loader = DataLoader(data, batch_size=128)
        ...
        class Net(nn.Module):
            ...
            def __init__(self, num_features, disconnected):
                ...
                self.output = nn.Sequential(nn.MaxPool1d(2), nn.ReLU())
        ...
        with torch.no_grad():
            diff = (
                (model(test_x1[:1], test_x2[:1]) - model(*background))
                .detach()
                .numpy()
                .mean(0)
            )
