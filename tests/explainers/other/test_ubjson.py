...
def test_decode_array():
    ...
    key_type = fp.read(1)
    key, value = _decode_simple_key_value_pair(fp, key_type)
    assert key == "left_children"
    assert value == np.array([-1], dtype=np.int32)
    ...
    key_type = fp.read(1)
    key, value = _decode_simple_key_value_pair(fp, key_type)
    assert key == "base_weights"
    assert value == [-0.0012426718603819609]
