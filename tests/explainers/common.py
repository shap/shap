    ...
    if len(shap_values.shape) == 3:
        ...
        for i in range(shap_values.shape[0]):
            ...
            if callable(explainer.masker.shape):
                all_on_masked = explainer.masker(
                    np.ones(explainer.masker.shape(data[i])[1], dtype=np.bool), data[i]
                )
            else:
                all_on_masked = explainer.masker(
                    np.ones(explainer.masker.shape[1], dtype=np.bool), data[i]
                )
        ...
        assert np.max(
            np.abs(shap_values.base_values + shap_values.values.sum(1) - model(data))
            < 1e6
        )
...
def test_interactions_additivity(explainer_type, model, masker, data, **kwargs):
    ...
    assert np.max(
        np.abs(shap_values.base_values + shap_values.values.sum((1, 2)) - model(data))
        < 1e6
    )
...
def test_serialization(
    explainer_type, model, masker, data, rtol=1e-05, atol=1e-8, **kwargs
):
    ...
    with tempfile.TemporaryFile() as temp_serialization_file:
        save_kwargs = {
            k: v for k, v in kwargs.items() if k in ["model_saver", "masker_saver"]
        }
        ...
        load_kwargs = {
            k: v for k, v in kwargs.items() if k in ["model_loader", "masker_loader"]
        }
    ...
    assert np.allclose(
        shap_values_original.base_values,
        shap_values_new.base_values,
        rtol=rtol,
        atol=atol,
    )
    assert np.allclose(
        shap_values_original[0].values, shap_values_new[0].values, rtol=rtol, atol=atol
    )
