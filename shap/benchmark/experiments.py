...
rf_regress_methods = ["tree_maple"]  # methods that only support random forest models
...
_experiments = []
_experiments += [
    ["corrgroups60", "lasso", m, s]
    for s in regression_metrics
    for m in linear_regress_methods
]
_experiments += [
    ["corrgroups60", "ridge", m, s]
    for s in regression_metrics
    for m in linear_regress_methods
]
_experiments += [
    ["corrgroups60", "decision_tree", m, s]
    for s in regression_metrics
    for m in tree_regress_methods
]
_experiments += [
...
_experiments += [
    ["corrgroups60", "gbm", m, s]
    for s in regression_metrics
    for m in tree_regress_methods
]
_experiments += [
    ["corrgroups60", "ffnn", m, s]
    for s in regression_metrics
    for m in deep_regress_methods
]

_experiments += [
    ["independentlinear60", "lasso", m, s]
    for s in regression_metrics
    for m in linear_regress_methods
]
_experiments += [
    ["independentlinear60", "ridge", m, s]
    for s in regression_metrics
    for m in linear_regress_methods
]
_experiments += [
    ["independentlinear60", "decision_tree", m, s]
    for s in regression_metrics
    for m in tree_regress_methods
]
_experiments += [
...
_experiments += [
    ["independentlinear60", "gbm", m, s]
    for s in regression_metrics
    for m in tree_regress_methods
]
_experiments += [
    ["independentlinear60", "ffnn", m, s]
    for s in regression_metrics
    for m in deep_regress_methods
]
...
_experiments += [
    ["cric", "lasso", m, s]
    for s in binary_classification_metrics
    for m in linear_classify_methods
]
_experiments += [
    ["cric", "ridge", m, s]
    for s in binary_classification_metrics
...
_experiments += [
    ["cric", "decision_tree", m, s]
    for s in binary_classification_metrics
    for m in tree_classify_methods
]
_experiments += [
    ["cric", "random_forest", m, s]
    for s in binary_classification_metrics
    for m in tree_classify_methods
]
_experiments += [
...
_experiments += [
    ["cric", "ffnn", m, s]
    for s in binary_classification_metrics
    for m in deep_classify_methods
]

_experiments += [
    ["human", "decision_tree", m, s]
    for s in human_metrics
    for m in tree_regress_methods
]
...
def run_experiment(experiment, use_cache=True, cache_dir="/tmp"):
    ...
    score = getattr(metrics, metric_name)(
        X, y, getattr(models, dataset_name + "__" + model_name), method_name
    )
...
def run_experiments(
    dataset=None, model=None, method=None, metric=None, cache_dir="/tmp", nworkers=1
):
    experiments_arr = list(
        experiments(dataset=dataset, model=model, method=method, metric=metric)
    )
    if nworkers == 1:
        out = list(
            map(
                run_experiments_helper,
                zip(experiments_arr, itertools.repeat(cache_dir)),
            )
        )
    else:
...
def __thread_worker(q, host):
    ...
    while True:
        ...
        __run_remote_experiment(
            experiment, hostname, cache_dir=cache_dir, python_binary=python_binary
        )
...
def run_remote_experiments(experiments, thread_hosts, rate_limit=10):
    ...
    for host in set(thread_hosts):
        ...
        try:
            subprocess.run(
                ["ssh", hostname, "pkill -f shap.benchmark.run_experiment"], timeout=15
            )
...
def __run_remote_experiment(
    experiment, remote, cache_dir="/tmp", python_binary="python"
):
    global total_failed
    dataset_name, model_name, method_name, metric_name = experiment
    ...
    cmd = (
        ...
        + python_binary
        + " -c \"import shap; shap.benchmark.run_experiment(['%s', '%s', '%s', '%s'], cache_dir='%s')\" &> %s/%s.output"
        % (
            dataset_name,
            model_name,
            method_name,
...
def __gen_cache_id(experiment):
    dataset_name, model_name, method_name, metric_name = experiment
    return "v" + "__".join(
        [__version__, dataset_name, model_name, method_name, metric_name]
    )
