...
def linear_shap_corr(model, data):
    ...
    return LinearExplainer(
        model, data, feature_dependence="correlation", nsamples=1000
    ).shap_values
...
def kernel_shap_1000_meanref(model, data):
    ...
    linestyle = solid
    """
    return lambda X: KernelExplainer(model.predict, kmeans(data, 1)).shap_values(
        X, nsamples=1000, l1_reg=0
    )
...
def sampling_shap_1000(model, data):
    ...
    linestyle = dashed
    """
    return lambda X: SamplingExplainer(model.predict, data).shap_values(
        X, nsamples=1000
    )
...
def tree_shap_independent_200(model, data):
    ...
    linestyle = dashed
    """
    data_subsample = sklearn.utils.resample(
        data, replace=False, n_samples=min(200, data.shape[0]), random_state=0
    )
    return TreeExplainer(
        model, data_subsample, feature_dependence="independent"
    ).shap_values
...
def lime_tabular_regression_1000(model, data):
    ...
    """
    return lambda X: other.LimeTabularExplainer(
        model.predict, data, mode="regression"
    ).attributions(X, nsamples=1000)
...
def lime_tabular_classification_1000(model, data):
    ...
    """
    return lambda X: other.LimeTabularExplainer(
        model.predict_proba, data, mode="classification"
    ).attributions(X, nsamples=1000)[1]
...
def maple(model, data):
    ...
    """
    return lambda X: other.MapleExplainer(model.predict, data).attributions(
        X, multiply_by_input=False
    )
...
def tree_maple(model, data):
    ...
    linestyle = dashed
    """
    return lambda X: other.TreeMapleExplainer(model, data).attributions(
        X, multiply_by_input=False
    )
