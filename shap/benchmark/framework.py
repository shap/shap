...
def update(model, attributions, X, y, masker, sort_order, perturbation_method, scores):
    ...
    sp = perturbation.SequentialPerturbation(
        model, masker, sort_order, perturbation_method
    )
...
def get_benchmark(model, attributions, X, y, masker, metrics):
    ...
    if safe_isinstance(X, "pandas.core.series.Series") or safe_isinstance(
        ...
        X = X.values
    if safe_isinstance(masker, "pandas.core.series.Series") or safe_isinstance(
        masker, "pandas.core.frame.DataFrame"
    ):
        masker = masker.values

    # record scores per metric
    scores = {"metrics": list(), "values": dict()}
    for sort_order, perturbation_method in list(
        it.product(metrics["sort_order"], metrics["perturbation"])
    ):
        update(
            model, attributions, X, y, masker, sort_order, perturbation_method, scores
        )
