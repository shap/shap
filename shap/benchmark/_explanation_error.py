...
class ExplanationError:
    ...
    def __call__(self, explanation, name, step_fraction=0.01, indices=[], silent=False):
        ...
        assert len(attributions) == len(self.model_args[0]), (
        ...
        for i, args in enumerate(zip(*self.model_args)):
            ...
            masked_model = MaskedModel(
                self.model, self.masker, self.link, self.linearize_link, *args
            )
            ...
            for _ in range(self.num_permutations):
                ...
                for j in range(0, feature_size, increment):
                    mask[
                        ordered_inds[np.arange(j, min(feature_size, j + increment))]
                    ] = True
                    masks.append(mask.copy())
                ...
                for l, v in enumerate(values):
                    values[l] = (
                        v - (base_value + np.sum(sample_attributions[masks_arr[l]]))
                    ) ** 2
