...
class ExplanationError:
    ...
    def __call__(self, explanation, name, step_fraction=0.01, indices=[], silent=False):
        ...
        for i, args in enumerate(zip(*self.model_args)):
            ...
            for _ in range(self.num_permutations):
                ...
                if row_clustering is not None:
                    ...
                    partition_tree_shuffle(ordered_inds, inds_mask, row_clustering, rng=self.rng)
                else:
                    self.rng.shuffle(ordered_inds)
        ...
        # BenchmarkResult("explanation error", name, value=np.sqrt(np.sum(total_values) / len(total_values)))
