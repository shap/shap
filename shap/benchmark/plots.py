...
def plot_human(dataset, model, metric, cmap=benchmark_color_map):
    ...
    pl.gca().xaxis.set_minor_formatter(
        matplotlib.ticker.FixedFormatter(["Fever", "Cough", "Headache"])
    )
    ...
    pl.gca().legend(
        ahandles, alabels, loc="upper center", bbox_to_anchor=(0.5, -0.15), ncol=2
    )
...
def _human_score_map(human_consensus, methods_attrs):
    ...
    v = 1 - min(
        np.sum(np.abs(methods_attrs - human_consensus))
        / (np.abs(human_consensus).sum() + 1),
        1.0,
    )
    return v
...
def make_grid(scores, dataset, model, normalize=True, transform=True):
    color_vals = {}
    metric_sort_order = {}
    for (_, _, method, metric), (fcounts, score) in filter(
        lambda x: x[0][0] == dataset and x[0][1] == model, scores
    ):
...
def plot_grids(dataset, model_names, out_dir=None):
    ...
    for ind, model in enumerate(model_names):
        ...
        for metric in col_keys:
            ...
            if save_plot:
                ...
                if out_dir is not None:
                    pl.savefig(
                        "%s/plot_%s_%s_%s.pdf" % (out_dir, dataset, model, metric),
                        format="pdf",
                    )
                pl.close()
                buf.seek(0)
                data_uri = (
                    base64.b64encode(buf.read()).decode("utf-8").replace("\n", "")
                )
                plot_id = "plot__" + dataset + "__" + model + "__" + metric
                prefix += (
                    "<div onclick='document.getElementById(\"%s\").style.display = \"none\"' style='display: none; position: fixed; z-index: 10000; left: 0px; right: 0px; top: 0px; bottom: 0px; background: rgba(255,255,255,0.9);' id='%s'>"
                    % (plot_id, plot_id)
                )
                prefix += (
                    "<img width='600' height='500' style='margin-left: auto; margin-right: auto; margin-top: 230px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);' src='data:image/png;base64,%s'>"
                    % data_uri
                )
                prefix += "</div>"

        model_title = (
            getattr(models, dataset + "__" + model).__doc__.split("\n")[0].strip()
        )
        ...
        if ind == 0:
            ...
            for j in range(data.shape[1]):
                metric_title = (
                    getattr(metrics, col_keys[j]).__doc__.split("\n")[0].strip()
                )
                out += (
        ...
        for i in range(data.shape[0]):
            ...
            for j in range(data.shape[1]):
                ...
                out += (
                    "<td onclick='document.getElementById(\"%s\").style.display = \"block\"' style='padding: 0px; padding-left: 0px; padding-right: 0px; border-left: 0px solid #999; width: 42px; min-width: 42px; height: 34px; background-color: #fff'>"
                    % plot_id
                )
                ...
                out += (
                    ...
                    + str(
                        tuple(
                            int(v * 255)
                            for v in colors.red_blue_no_bounds(5 * (data[i, j] - 0.8))[
                                :-1
                            ]
                        )
                    )
    ...
    for i in range(legend_size - 9):
        ...
        out += (
            ...
            + str(
                tuple(int(v * 255) for v in colors.red_blue_no_bounds(5 * (val - 0.8)))[
                    :-1
                ]
            )
            + "; height: "
