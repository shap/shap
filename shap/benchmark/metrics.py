...
def runtime(X, y, model_generator, method_name):
    ...
    for i in range(3):
        X_train, X_test, y_train, _ = train_test_split(
            __toarray(X), y, test_size=100, random_state=i
        )
...
def local_accuracy(X, y, model_generator, method_name):
    ...
    def score_function(
        X_train, X_test, y_train, y_test, attr_function, trained_model, random_state
    ):
        return measures.local_accuracy(
            ...
            X_test,
            y_test,
            attr_function(X_test),
            model_generator,
            score_map,
            trained_model,
        )

    return None, __score_method(
        X, y, None, model_generator, score_function, method_name
    )
...
def remove_absolute_resample__roc_auc(
    X, y, model_generator, method_name, num_fcounts=11
):
    """Remove Absolute (resample)
    xlabel = "Max fraction of features removed"
    ylabel = "1 - ROC AUC"
...
def __run_measure(
    ...
    return fcounts, __score_method(
        X, y, fcounts, model_generator, score_function, method_name
    )


def batch_remove_absolute_retrain__r2(
    X, y, model_generator, method_name, num_fcounts=11
):
...
def batch_remove_absolute_retrain__roc_auc(
    X, y, model_generator, method_name, num_fcounts=11
):
    """Batch Remove Absolute (retrain)
    xlabel = "Fraction of features removed"
...
def batch_keep_absolute_retrain__roc_auc(
    X, y, model_generator, method_name, num_fcounts=11
):
    """Batch Keep Absolute (retrain)
    xlabel = "Fraction of features kept"
...
def __run_batch_abs_metric(
    metric, X, y, model_generator, method_name, loss, num_fcounts
):
    def score_function(
        fcount, X_train, X_test, y_train, y_test, attr_function, trained_model
    ):
...
def __score_method(
    ...
    try:
        ...
        assert (
            False
        ), "The 'dill' package could not be loaded and is needed for the benchmark!"
    ...
    data_hash = (
        hashlib.sha256(__toarray(X).flatten()).hexdigest()
        + hashlib.sha256(__toarray(y)).hexdigest()
    )
    for i in range(nreps):
        X_train, X_test, y_train, y_test = train_test_split(
            __toarray(X), y, test_size=test_size, random_state=i
        )

        # define the model we are going to explain, caching so we onlu build it once
        model_id = (
            "model_cache__v"
            + "__".join([__version__, data_hash, model_generator.__name__])
            + ".pickle"
        )
        ...
        def score(attr_function):
            ...
            if fcounts is None:
                return score_function(
                    X_train, X_test, y_train, y_test, cached_attr_function, model, i
                )
            else:
                scores = []
                for f in fcounts:
...
def __intlogspace(start, end, count):
    return np.unique(
        np.round(
            start + (end - start) * (np.logspace(0, 1, count, endpoint=True) - 1) / 9
        ).astype(np.int)
    )
...
def _human_and(X, model_generator, method_name, fever, cough):
    assert (
        np.abs(X).max() == 0
    ), "Human agreement metrics are only for use with the human_agreement dataset!"
