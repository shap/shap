...
class SequentialMasker:
    def __init__(
        self, mask_type, sort_order, masker, model, *model_args, batch_size=500
    ):

        for arg in model_args:
            if isinstance(arg, pd.DataFrame):
                raise Exception(
                    "DataFrame arguments dont iterate correctly, pass numpy arrays instead!"
                )
    ...
    def __call__(self, explanation, name, **kwargs):
        return self.inner(
            name, explanation, *self.model_args, batch_size=self.batch_size, **kwargs
        )
...
class SequentialPerturbation:
    def __init__(self, model, masker, sort_order, perturbation, linearize_link=False):
        ...
        if self.sort_order == "positive":
            ...
            raise ValueError(
                'sort_order must be either "positive", "negative", or "absolute"!'
            )

        # user must give valid masker
        underlying_masker = (
            masker.masker if isinstance(masker, FixedComposite) else masker
        )
        ...
        *model_args,
        ...
        for i, args in enumerate(zip(*model_args)):
            ...
            self.masked_model = MaskedModel(
                self.model, self.masker, links.identity, self.linearize_link, *args
            )

            masks = []

            mask = np.ones(feature_size, dtype=np.bool) * (
                self.perturbation == "remove"
            )
            ...
            for j in range(0, feature_size, increment):
                oind_list = [
                    ordered_inds[l] for l in range(j, min(feature_size, j + increment))
                ]

                for oind in oind_list:
                    if not (
                        (
                            self.sort_order == "positive"
                            and sample_attributions[oind] <= 0
                        )
                        or (
                            self.sort_order == "negative"
                            and sample_attributions[oind] >= 0
                        )
                    ):
        ...
        if not debug_mode:
            return BenchmarkResult(
                self.perturbation + " " + self.sort_order,
                name,
                curve_x=xs,
                curve_y=ys,
                curve_y_std=std,
            ...
            for j in range(len(self.score_values[-1])):
                ...
                auc = sklearn.metrics.auc(
                    np.linspace(0, 1, len(curve)), curve_sign * (curve - curve[0])
                )
                aucs.append(auc)
    ...
    def score(
        ...
        if debug_mode:
            ...
            auc = sklearn.metrics.auc(
                np.linspace(0, 1, len(ys)), curve_sign * (ys - ys[0])
            )
            return xs, ys, auc
