...
class TeacherForcing(Model):
    ...
    def __init__(
        ...
        if (
            ...
            and getattr_silent(self.tokenizer, "pad_token") is None
        ):
        ...
        if safe_isinstance(model, "transformers.PreTrainedModel") or safe_isinstance(
            ...
            self.text_generate = models.TextGeneration(
                self.inner_model, tokenizer=self.tokenizer, device=self.device
            )
            ...
            self.text_generate = models.TextGeneration(
                self.inner_model, device=self.device
            )
            ...
            if (
                self.similarity_tokenizer is not None
                and self.similarity_tokenizer.pad_token is None
            ):
                self.similarity_tokenizer.pad_token = (
                    self.similarity_tokenizer.eos_token
                )
    ...
    def get_output_names(self, output):
        ...
        output_names = [
            self.similarity_tokenizer.decode([x]).strip() for x in output_ids[0, :]
        ]
        return output_names
    ...
    def get_outputs(self, X):
        ...
        if X.dtype.type is np.str_:
            ...
            if keep_suffix > 0:
                output_ids = np.array(
                    self.similarity_tokenizer(X.tolist(), padding=True)["input_ids"]
                )[:, keep_prefix:-keep_suffix]
            else:
                output_ids = np.array(
                    self.similarity_tokenizer(X.tolist(), padding=True)["input_ids"]
    ...
    def get_inputs(self, X, padding_side="right"):
        ...
        inputs = self.similarity_tokenizer(
            input_sentences.tolist(),
            return_tensors=self.similarity_model_type,
            padding=True,
        )
    ...
    def get_logodds(self, logits):
        ...
        logodds_for_output_ids = logodds[
            :, np.array(range(logodds.shape[1])), output_ids
        ]
    ...
    def model_inference(self, inputs, output_ids):
        ...
        if self.similarity_model_type == "pt":
            ...
            if self.similarity_model.config.is_encoder_decoder:
                ...
                # combine source and target sentence ids to pass into decoder eg: in case of distillgpt2
                inputs["input_ids"] = tf.concat(
                    (inputs["input_ids"], output_ids), axis=-1
                )
                attention_mask_for_output_ids = tf.ones(
                    output_ids.shape, dtype=output_ids.dtype
                )
                inputs["attention_mask"] = tf.concat(
                    (inputs["attention_mask"], attention_mask_for_output_ids), axis=-1
                )
                inputs["position_ids"] = (
    ...
    def load(cls, in_file, instantiate=True):
        ...
        with Deserializer(
            in_file, "shap.models.TeacherForcing", min_version=0, max_version=0
        ) as s:
