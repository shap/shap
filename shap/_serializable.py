...
class Serializable:
    ...
    def _instantiated_load(cls, in_file, **kwargs):
        ...
        if not inspect.isclass(obj_type) or (
            not issubclass(obj_type, cls) and (obj_type is not cls)
        ):
            raise Exception(
                f"Invalid object type loaded from file. {obj_type} is not a subclass of {cls}."
            )
        ...
        return obj_type(
            **{k: constructor_args[k] for k in constructor_args if k in used_args}
        )
...
class Serializer:
    ...
    def save(self, name, value, encoder="auto"):
        ...
        if encoder is None or encoder is False:
        ...
        elif encoder == ".save" or (
            isinstance(value, Serializable) and encoder == "auto"
        ):
            log.debug("encoder_name = %s", "serializable.save")
            pickle.dump("serializable.save", self.out_stream)
            if (
                len(inspect.getfullargspec(value.save)[0]) == 3
            ):  # backward compat for MLflow, can remove 4/1/2021
            ...
            raise ValueError(
                f"Unknown encoder type '{encoder}' given for serialization!"
            )
...
class Deserializer:
    ...
    def _load_data_value(self, decoder=None):
        ...
        if encoder_name == "custom_encoder" or callable(decoder):
            assert callable(
                decoder
            ), "You must provide a callable custom decoder for the data item {name}!"
