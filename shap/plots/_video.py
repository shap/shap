...
def video(shap_values: Explanation or np.ndarray,
    ...
    shap_values : [numpy.array]
        ...
        (# samples x channels x time x width x height), and the
        length of the list is equal to the number of model outputs that are being
        explained.

    pixel_values : numpy.array
        Matrix of pixel values (# samples x channels x time x width x height ) for each video.
    ...
    x = pixel_values
    fig_size = np.array([3 * (len(shap_values) + 1), 2 * 2.5 * (x.shape[0] + 1)])
    if fig_size[0] > width:
    ...
    for row in range(x.shape[0]):
        ...
        # make sure we have a 3D array for grayscale
        if len(x_curr.shape) == 4 and x_curr.shape[0] == 1:
            x_curr = x_curr.reshape(x_curr.shape[1:])

        # if x_curr.max() > 1:
        #     x_curr /= 255.

        # get a grayscale version of the image
        if len(x_curr.shape) == 4 and x_curr.shape[0] == 3:
            x_curr_gray = (
            ...
            x_curr_disp[x_curr_disp < 0] = 0 # THW
        else:
            x_curr_gray = x_curr # ?
            x_curr_disp = x_curr # ?

        # axes[row, 0].imshow(x_curr_disp[0], cmap=pl.get_cmap('gray'))
        frames = []
        ...
        if len(shap_values[0][row].shape) == 2:
            ...
            abs_vals = np.stack([np.abs(shap_values[i].sum(0)) for i in range(len(shap_values))], 0).flatten()
        max_val = np.nanpercentile(abs_vals, 99.9)
        for i in range(len(shap_values)):
            ax = fig.add_subplot(row+1, len(shap_values), i + 1, projection="3d")

            if labels is not None:
                ax.set_title(labels[row, i], **label_kwargs)
            sv = shap_values[i][row] if len(shap_values[i][row].shape) == 2 else shap_values[i][row].sum(0)
            # axes[row, i + 1].imshow(x_curr_gray, cmap=pl.get_cmap('gray'), alpha=0.15,
                                    # extent=(-1, sv.shape[1], sv.shape[0], -1))
            face_color = cmap(sv+max_val / (2 * max_val))
            im = ax.voxels(sv, facecolors=face_color) #, vmin=-max_val, vmax=max_val)
            ax.axis('off')
    if hspace == 'auto':
        fig.tight_layout()
    else:
