...
def waterfall(shap_values, max_display=10, show=True):
    ...
    for i in range(len(pos_inds)):
        ...
        if pos_low is not None and i < len(pos_low):
            plt.errorbar(
                ...
                ecolor=STYLE.default_positive_color,
            )
    ...
    for i in range(len(neg_inds)):
        ...
        if neg_low is not None and i < len(neg_low):
            plt.errorbar(
                ...
                ecolor=STYLE.default_negative_color,
            )
    ...
    for i in range(num_features):
        plt.axhline(i, color=STYLE.hlines, lw=0.5, dashes=(1, 5), zorder=-1)

    # mark the prior expected value and the model prediction
    plt.axvline(base_values, 0, 1 / num_features, color=STYLE.vlines, linestyle="--", linewidth=0.5, zorder=-1)
    fx = base_values + values.sum()
    plt.axvline(fx, 0, 1, color=STYLE.vlines, linestyle="--", linewidth=0.5, zorder=-1)
    ...
    tick_labels[1].set_color(STYLE.tick_labels)
    ...
    for i in range(num_features):
        tick_labels[i].set_color(STYLE.tick_labels)
...
def waterfall_legacy(expected_value, shap_values=None, features=None, feature_names=None, max_display=10, show=True):
    ...
    # mark the prior expected value and the model prediction
    plt.axvline(expected_value, 0, 1 / num_features, color=STYLE.vlines, linestyle="--", linewidth=0.5, zorder=-1)
