...
def waterfall(shap_values, max_display=10, show=True, waterfall_clr=None):
    ...
    waterfall_clr: a dictionary specifying colors in either as string vale of rgb value
        example ---
        waterfall_clr = {"c1_rgb":"darkgreen",
                        "light_c1_rgb":"green",
                        "c2_rgb":[1.,0.,0.31796406],
                        "light_c2_rgb":[1.,0.49803922,0.65490196]}
        where c1 -> color to show increase (default:red)
              c2 -> color to show decrease (default:blue)
    """
    ...
    if waterfall_clr:
        ...
        colors = pd.DataFrame(
            [list(waterfall_clr.values())], columns=list(waterfall_clr.keys())
        ).iloc[0]
