...
def waterfall(shap_values, max_display=10, show=True, ax=None):
    ...
    ax : matplotlib Axes, optional
        Optionally specify an existing :external+mpl:class:`~matplotlib.axes.Axes` object, into which the
        plot will be placed.

        This argument is placed after ``show`` for backward compatibility with
    ...
    if ax is None:
        ax = plt.gca()
        fig = ax.get_figure()
        # Match historical behavior by resizing the current figure only when ax is not user-supplied.
    ...
    if show:
        ...
        # Return the primary plotting axis. The twinned axes above are auxiliary tick-label axes.
        return ax
