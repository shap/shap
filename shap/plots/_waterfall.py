...
def waterfall(shap_values, max_display=10, show=True, ax=None):
    ...
    ax : matplotlib.axes.Axes or None
        Axes object to draw the plot onto. If ``None`` (default), the current axes are
        used, and the figure is resized to fit the number of features. Pass an explicit
        ``Axes`` to embed the waterfall plot in a subplot grid.
    ...
    if ax is None:
        ax = plt.gca()
        ax.figure.set_size_inches(8, num_features * row_height + 1.5)
