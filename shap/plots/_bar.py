...
def bar(
        ...
        if len(inds) == 1:
            ...
            if len(full_print) <= 40:
                ...
                feature_names_new.append(
                    feature_names[inds[max_ind]]
                    + " + %d other features" % (len(inds) - 1)
                )
    ...
    for i in feature_inds:
        if features is not None and show_data:
            yticklabels.append(
                format_value(features[i], "%0.03f") + " = " + feature_names[i]
            )
        else:
...
def bar_legacy(
    shap_values, features=None, feature_names=None, max_display=None, show=True
):
