...
class MaskedModel:
        ...
        if len(masks.shape) == 1:
            if getattr(self.masker, "supports_delta_masking", False):
                return self._delta_masking_call(
                    masks, zero_index=zero_index, batch_size=batch_size
                )
            ...
            # input but the masker does not support delta masking
                ...
                full_masks = np.zeros(
                    (int(np.sum(masks >= 0)), self._masker_cols), dtype=np.bool
                )
                _convert_delta_mask_to_full(masks, full_masks)
                return self._full_masking_call(
                    full_masks, zero_index=zero_index, batch_size=batch_size
                )
    ...
    def _full_masking_call(self, masks, zero_index=None, batch_size=None):
        ...
        for batch_ind in range(0, len(masks), batch_size):
            ...
            for i, mask in enumerate(mask_batch):
                ...
                if i == 0 or self._variants is None:
                    ...
                    varying_rows.append(
                        np.any(delta_tmp, axis=1)
                    )  # np.any(self._variants & delta_mask, axis=1))
                    num_varying_rows[batch_ind + i] = varying_rows[-1].sum()
                    # for i in range(20):
                    #     varying_rows[-1].sum()
                last_mask[:] = mask

                batch_positions[batch_ind + i + 1] = (
                    batch_positions[batch_ind + i] + num_varying_rows[batch_ind + i]
                )
                ...
                if num_varying_rows[batch_ind + i] != num_mask_samples[i]:
                    if len(self.args) == 1:
                        ...
                        masked_inputs_subset = [
                            v[varying_rows[-1]] for v in zip(*masked_inputs[0])
                        ]
        ...
        if (
            self.linearize_link
            and self.link != links.identity
            and self._linearizing_weights is None
        ):
            self.background_outputs = outputs[
                batch_positions[zero_index] : batch_positions[zero_index + 1]
            ]
            self._linearizing_weights = link_reweighting(
                self.background_outputs, self.link
            )

        averaged_outs = np.zeros((len(batch_positions) - 1,) + outputs.shape[1:])
        max_outs = (
            self._masker_rows
            if self._masker_rows is not None
            else max(len(r) for r in varying_rows)
        )
    ...
    def _delta_masking_call(self, masks, zero_index=None, batch_size=None):
        ...
        assert (
            getattr(self.masker, "supports_delta_masking", None) is not None
        ), "Masker must support delta masking!"
    ...
    def mask_shapes(self):
        if hasattr(self.masker, "mask_shapes") and callable(self.masker.mask_shapes):
            ...
            return [
                a.shape for a in self.args
            ]  # TODO: this will need to get more flexible
...
def _build_delta_masked_inputs(
    ...
    while masks_pos < len(masks):
        ...
        if i == 0:
            ...
            # more than one column was changed
                ...
                varying_rows[i, :] = np.any(
                    variants[:, delta_indexes[: dpos + 1]], axis=1
                )
    ...
    return (
        all_masked_inputs,
        i + 1,
    )  # i + 1 is the number of output rows after averaging
    ...
    for i in range(0, len(averaged_outs)):
        if batch_positions[i] < batch_positions[i + 1]:
            if num_varying_rows[i] == sample_count:
                ...
                last_outs[varying_rows[i]] = outputs[
                    batch_positions[i] : batch_positions[i + 1]
                ]
            if linearizing_weights is not None:
        ...
        if batch_positions[i] < batch_positions[i + 1]:
            ...
            if linearizing_weights is not None:
                for j in range(last_outs.shape[-1]):
                    averaged_outs[i, j] = np.mean(
                        linearizing_weights[:, j] * link(last_outs[:, j])
                    )
            else:
...
def make_masks(cluster_matrix):
    ...
    mask_matrix = scipy.sparse.csr_matrix(
        (np.ones(len(indices), dtype=np.bool), indices, indptr), shape=(2 * M - 1, M)
    )
...
def link_reweighting(p, link):
    ...
    weights = (linearized_link - link(expected_value)) / (
        link(p) - link(expected_value)
    )
