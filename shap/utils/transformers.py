...
def parse_prefix_suffix_for_tokenizer(tokenizer):
    ...
    if len(null_tokens) == 1:
        ...
        if hasattr(tokenizer, "special_tokens_map") and hasattr(tokenizer, "decode"):
            ...
            assert ("eos_token" in st_map) or (
                "bos_token" in st_map
            ), "No eos token or bos token found in tokenizer!"
            if ("eos_token" in st_map) and (
                tokenizer.decode(null_token) == st_map["eos_token"]
            ):
                keep_prefix = 0
                keep_suffix = 1
                # prefix_strlen = 0
                # suffix_strlen = len(tokenizer.decode(null_tokens[-keep_suffix:]))
            elif ("bos_token" in st_map) and (
                tokenizer.decode(null_token) == st_map["bos_token"]
            ):
                keep_prefix = 1
                keep_suffix = 0
        ...
        assert (
            len(null_tokens) % 2 == 0
        ), "An odd number of boundary tokens are added to the null string!"
