...
class TreeExplainer(Explainer):
    ...
    def __init__(
        ...
        if self.data is None:
            ...
            warnings.warn(
                "Passing " + str(self.data.shape[0]) + " background samples may lead to slow runtimes. Consider "
    ...
    def shap_interaction_values(self, X, y=None, tree_limit=None):
        ...
        if (
            ...
            if tree_limit == -1:
                tree_limit = 0
            xgb_tree_limit = tree_limit // self.model.num_stacked_models
            phi = self.model.original_model.predict(X, iteration_range=(0, xgb_tree_limit), pred_interactions=True,
                                                    validate_features=False)
        ...
        def check_sum(sum_val, model_output):
            ...
            if np.max(diff / (np.abs(sum_val) + 1e-2)) > 1e-2:
                ...
                err_msg += " This check failed because for one of the samples the sum of the SHAP values" \
                           ...
                           " you can set check_additivity=False to disable this check.".format(sum_val[ind],
                                                                                               model_output[ind])
        ...
        # deprecated dictionary support (with sklearn singular style "feature" and "value" names)
            ...
            def buildTree(index, node):
                ...
                self.node_sample_weight[
                    index] = node.impurityStats().count()  # weighted count of element through this node
