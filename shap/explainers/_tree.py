...
class TreeExplainer(Explainer):
    ...
    def _validate_inputs(
        ...
        npt.NDArray[Any],
        npt.NDArray[Any] | pd.Series | None,
    ...
    def assert_additivity(
        self,
        phi: npt.NDArray[Any] | list[npt.NDArray[Any]],
        model_output: npt.NDArray[Any],
    ) -> None:
...
class TreeliteModelLoader:
    ...
    def _parse_tree(self, treelite_model: Any, tree_id: int) -> dict:
        ...
        * cmp=5 (>=) : swap cleft/cright and shift threshold up by 1 ULP
        ...
        # cmp=3 (<=) with float32 source (XGBoost): apply same 1-ULP shift as
        # SHAP's XGBoost loader for threshold parity.
        if threshold_raw.dtype == np.float32:
            threshold = np.where(
                is_internal & (cmp == 3),
