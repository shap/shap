...
class TFDeepExplainer(Explainer):
    ...
    def __init__(self, model, data, session=None, learning_phase_flags=None):
        ...
        if str(type(model)).endswith("keras.engine.sequential.Sequential'>"):
            ...
            self.model_inputs = model.inputs
            assert len(model.outputs) == 1, "Multiple output models are not currently supported by DeepExplainer! " \
                                            "Wrap as a single output model before explaining."
        ...
        if self.phi_symbolics[i] is None:
            ...
            if not tf.executing_eagerly():
                ...
                def grad_graph(shap_rAnD):
                    ...
                    self.between_ops = forward_walk_ops(
                        [op for t in shap_rAnD for op in t.consumers()],
                        tensor_blacklist, dependence_breakers,
                        within_ops=back_ops
                    )
...
def nonlinearity_1d_handler(input_ind, explainer, op, *grads):
    ...
    # strangely sometimes TF does not pass us the inputs on the _MockOp, so here we get them another way
    if inputs is None and outputs is not None:
        inputs = op.outputs[0].op.inputs

    # make sure only the given input varies
    for i in range(len(inputs)):
        if i != input_ind:
            assert not explainer._variable_inputs(op)[i], str(i) + "the input to " + op.name + " cannot vary!"
    
    xin0,rin0 = tf.split(inputs[input_ind], 2)
    xout,rout = tf.split(outputs[input_ind], 2)
    delta_in0 = xin0 - rin0
    dup0 = [2] + [1 for i in delta_in0.shape[1:]]
    out = [None for _ in inputs]
    orig_grads = explainer.orig_grads[op.type](op, grads[0])
    out[input_ind] = tf.where(
        tf.tile(tf.abs(delta_in0), dup0) < 1e-6,
        orig_grads[input_ind] if len(inputs) > 1 else orig_grads,
