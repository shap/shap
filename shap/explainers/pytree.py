...
class TreeExplainer:
    ...
    def __init__(self, model, **kwargs):
        ...
        if str(type(model)).endswith("sklearn.ensemble.forest.RandomForestRegressor'>"):
            ...
            raise Exception(
                "Model type not supported by TreeExplainer: " + str(type(model))
            )
    ...
    def shap_interaction_values(self, X, tree_limit=-1, **kwargs):
        ...
        if self.model_type == "xgboost":
            ...
            raise Exception(
                "Interaction values not yet supported for model type: " + str(type(X))
            )
...
def extend_path(
    ...
    for i in range(unique_depth - 1, -1, -1):
        ...
        pweights[i] = (
            zero_fraction * pweights[i] * (unique_depth - i) / (unique_depth + 1.0)
        )
...
def unwind_path(
    ...
    for i in range(unique_depth - 1, -1, -1):
        if one_fraction != 0.0:
            tmp = pweights[i]
            pweights[i] = (
                next_one_portion * (unique_depth + 1.0) / ((i + 1.0) * one_fraction)
            )
