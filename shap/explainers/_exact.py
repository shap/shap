...
class Exact(Explainer):
    ...
    def __init__(
        ...
        if getattr(masker, "clustering", None) is not None:
            self._partition_masks, self._partition_masks_inds = partition_masks(
                masker.clustering
            )
            self._partition_delta_indexes = partition_delta_indexes(
                masker.clustering, self._partition_masks
            )

        self._gray_code_cache = (
            {}
        )  # used to avoid regenerating the same gray code patterns
    ...
    def explain_row(
        ...
        *row_args,
        ...
        if getattr(self.masker, "clustering", None) is None:
            ...
            # Shapley values
            if (
                interactions is False or interactions is 1
            ...
            # Shapley-Taylor interaction values
                ...
                interactions is True or interactions is 2
            ):  # pylint: disable=literal-comparison
        ...
        if (
            main_effects or interactions is True or interactions is 2
        ):  # pylint: disable=literal-comparison
            if inds is None:
...
def _compute_grey_code_row_values(
    row_values, mask, inds, outputs, shapley_coeff, extended_delta_indexes, noop_code
):
    set_size = 0
    M = len(inds)
    for i in range(2**M):
...
def partition_masks(partition_tree):
    ...
    return all_masks[order], np.array(
        [[np.array(on), np.array(off)] for on, off in inds_lists]
    )
