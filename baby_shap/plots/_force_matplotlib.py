...
def draw_bars(out_value, features, feature_type, width_separators, width_bar):
    ...
    for index, features in zip(range(len(features)), features):
        ...
        line = plt.Polygon(
            points_separator, closed=None, fill=None, edgecolor=colors[1], lw=3
        )
        separator_list += [line]
...
def draw_labels(
    ...
    for feature in features:
        ...
        feature_contribution = np.abs(float(feature[0]) - pre_val) / np.abs(
            total_effect
        )
...
def update_axis_limits(
    ax, total_pos, pos_features, total_neg, neg_features, base_value
):
...
def draw_additive_plot(data, figsize, show, text_rotation=0, min_perc=0.05):
    ...
    # Compute axis limit
    update_axis_limits(ax, total_pos, pos_features, total_neg, neg_features, base_value)
    ...
    rectangle_list, separator_list = draw_bars(
